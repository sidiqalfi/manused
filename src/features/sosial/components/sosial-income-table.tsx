...
export function SosialIncomeTable({ periodId, period, summary, members }: Props) {
  ...
  async function handleDelete(incomeId: string) {
    const result = await deleteSosialIncome(incomeId)
    if (result.error) {
  ...
  const minAmount = summary?.success ? summary.data?.minAmount ?? 2000 : 2000
          ...
          <tbody>
            ...
            {incomes.map((income) => (
              <tr key={income.id} className="border-b">
                ...
                <td className="p-3">
                  <Badge variant="default">Sudah Iuran</Badge>
                </td>
      ...
      {unpaidMembers.length > 0 && (
        <div>
          <p className="mb-2 text-sm text-muted-foreground">
            Belum iuran ({unpaidMembers.length} anggota):
          </p>
